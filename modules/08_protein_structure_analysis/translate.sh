...
# Usage: bash translate.sh --input <nucleotide.fa> --output <protein.fa> --threads N

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MODULES="$(cd "$SCRIPT_DIR/.." && pwd)"
...
command -v transeq &>/dev/null || { log_error "transeq not found — install EMBOSS (conda install -c bioconda emboss)"; exit 1; }
...
log_info "Translation complete: $(basename "$OUTPUT_FASTA")"
