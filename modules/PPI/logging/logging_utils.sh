...
setup_logging() {
	...
	# Export all file paths so they're available in subshells and after directory changes
	export LOG_FILE="${LOG_DIR}/pipeline_${RUN_ID}_full_log.log"
	export TIME_FILE="${TIME_DIR}/pipeline_${RUN_ID}_time_metrics.csv"
	export TIME_TEMP="${TIME_DIR}/.time_temp_${RUN_ID}.txt"
	export SPACE_FILE="${SPACE_DIR}/pipeline_${RUN_ID}_space_metrics.csv"
	export SPACE_TIME_FILE="${SPACE_TIME_DIR}/pipeline_${RUN_ID}_combined_metrics.csv"
	export ERROR_WARN_FILE="${ERROR_WARN_DIR}/pipeline_${RUN_ID}_errors_warnings.log"
	export SOFTWARE_FILE="${SOFTWARE_CATALOG_DIR}/software_catalog_${RUN_ID}.csv"
	export GPU_LOG_FILE="${GPU_LOG_DIR}/gpu_${RUN_ID}.log"
