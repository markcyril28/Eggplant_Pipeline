...
#
# Edit i_crispr_v1CONFIG.toml to change gene groups, compute settings, and
# operations, then run:
#   bash i_crispr_v1.sh
# ============================================================================
...
# from i_crispr_v1CONFIG.toml [pipeline] — edit gene_groups there.
# ===============================================================
...
SHARED_CONFIG="$PIPELINE_DIR/i_crispr_v1CONFIG.toml"
mapfile -t GENE_GROUPS < <(python3 "$TOML_PARSER" "$SHARED_CONFIG" pipeline gene_groups 2>/dev/null)
if [[ ${#GENE_GROUPS[@]} -eq 0 ]]; then
    echo "ERROR: pipeline.gene_groups is empty in i_crispr_v1CONFIG.toml" >&2
    exit 1
fi
...
if [[ -d "$CONFIG_DIR" ]]; then
    ...
    cat "$PIPELINE_DIR/i_crispr_v1CONFIG.toml" \
