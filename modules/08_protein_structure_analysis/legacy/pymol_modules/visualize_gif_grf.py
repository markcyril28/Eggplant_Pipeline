#!/usr/bin/env python3
"""
PyMOL Visualization for SmelGRF-GIF Interaction
Target: fold_1_x_1 (SmelGRF-GIF complex)

This script visualizes the SmelGRF-GIF protein interaction with:
  - Chain identification via sequence alignment
  - Interface residue highlighting (hydrophobic contacts)
  - Custom bubble surfaces around key residues
  - Multiple output formats (Black/White/Transparent)

Usage:
  pymol -c visualize_gif_grf.py
  pymol -c visualize_gif_grf.py -- --skip
"""

print("\n" + "="*60, flush=True)
print("Loading SmelGRF-GIF Visualization Pipeline...", flush=True)
print("="*60 + "\n", flush=True)
...
from pymol_config_gif_grf import (
...
def setup_logging():
    ...
    log_filename = os.path.join(logs_dir, f"pymol_gif_grf_{timestamp}.log")
...
def visualize_fold(fold_name, skip_existing=False):
    """Visualize the SmelGRF-GIF interaction fold."""
    logging.info("="*60)
    logging.info(f"Processing: {fold_name} (SmelGRF-GIF)")
    logging.info("="*60)
...
def main():
    ...
    print("\n" + "="*60, flush=True)
    print("SmelGRF-GIF Visualization Pipeline", flush=True)
    print("="*60 + "\n", flush=True)
    ...
    logging.info("="*60)
    logging.info("SmelGRF-GIF Visualization Pipeline")
    logging.info("="*60)
    ...
    if result:
        print("\n" + "="*60, flush=True)
        print("[OK] SmelGRF-GIF visualization complete!", flush=True)
        print(f"Log saved to: {log_file}", flush=True)
        print("="*60 + "\n", flush=True)
        
        logging.info("="*60)
        logging.info("[OK] SmelGRF-GIF visualization complete!")
