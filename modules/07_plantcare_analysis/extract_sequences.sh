...
#
# FASTA HEADER FORMAT:
#   >{GeneID}:{Chromosome}:{CDS_Start}-{CDS_End}:{Strand} region={Chr}:{ExtractStart}-{ExtractEnd} upstream={UPSTREAMbp}bp downstream={DOWNSTREAMbp}bp
#   Example: >AtGRF1:chr1:15230-17891:+ region=chr1:14230-18891 upstream=1000bp downstream=1000bp
#
# Conventions:
...
while IFS=$'\t' read -r GeneName Col2 ChromosomeNumber LowerNumber UpperNumber Strand Rest; do
    ...
    (
        ...
        # Format: >GeneID:chr:cds_start-cds_end:strand region=chr:genomic_start-genomic_end upstream=Nbp downstream=Mbp
        awk -v gene="$GeneName" -v chr="$ChromosomeNumber" -v cds_start="$LowerNumber" \
            ...
            'NR==1 {
                print ">" gene ":" chr ":" cds_start "-" cds_end ":" strand " region=" chr ":" start "-" end " upstream=" up "bp downstream=" down "bp"
                next
            }
            {
                gsub(/[[:space:]]/, "", $0)
