#!/usr/bin/env Rscript
# Create heatmap with functions organized by major groups - horizontal labels
...
if (length(args) < 3) {
    stop("Usage: Rscript create_function_groups_heatmap_horizontal.R <matrix.tsv> <output_dir> <dpi> [color_palette] [row_font] [col_font] [cell_font] [cell_size] [label_font] [cell_border_color] [cell_border_width] [column_rotation] [legend_height] [gene_label]")
}
...
# Create labels for major groups with alternating vertical positions
group_labels_top <- rep("", ncol(mat))
group_labels_bottom <- rep("", ncol(mat))
current_pos <- 1
for (i in 1:nrow(group_info)) {
    ...
    if (mid_pos <= ncol(mat)) {
        clean_name <- gsub("-\n\\s*", "-\n", group_name)
        # Alternate: odd groups on top row, even groups on bottom row
        if (i %% 2 == 1) {
            group_labels_top[mid_pos] <- clean_name
        } else {
            group_labels_bottom[mid_pos] <- clean_name
        }
    }
    current_pos <- current_pos + func_count
}

# Create custom column labels annotation - two rows for alternating heights
col_anno <- HeatmapAnnotation(
    group_label_top = anno_text(
        group_labels_top,
        rot = 0,
        location = 0.5,
        just = "center",
    ...
    which = "column",
    gap = unit(0.000005, "mm"),
    annotation_label = "Major Groups",
    annotation_name_side = "left",
    annotation_name_gp = gpar(fontsize = label_font, fontface = "bold"),
...
# Save
output_file <- file.path(output_dir, "function_groups_heatmap_horizontal.png")
png(output_file, width = 10 + ncol(mat) * 0.2, height = 8 + nrow(mat) * 0.4, 
    units = "in", res = dpi)
draw(ht, padding = unit(c(0, 0.25, 0.5, 0.05), "inches"))
dev.off()
